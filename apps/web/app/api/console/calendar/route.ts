...
export async function POST(request: Request) {
  ...
  const parsed = await readJsonRequest<Record<string, unknown>>(request, 131_072, "Calendar");
  if (!parsed.ok) return parsed.response;
  return forwardDeskRequest("/api/v1/calendar", { method: "POST", body: parsed.body });
}
