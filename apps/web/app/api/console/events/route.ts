...
import { demoOpenEvents } from "../../../../lib/console-demo";

export async function GET(request: Request) {
  if (!authenticateConsoleRequest(request)) return consoleAuthFailure(true);
  if (process.env.EASTMED_CONSOLE_DEMO === "true") return Response.json(demoOpenEvents);
  return forwardDeskRequest("/api/v1/events/open");
}
